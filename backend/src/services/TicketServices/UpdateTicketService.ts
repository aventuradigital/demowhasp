    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      ...
      if (setting?.value === "enabled") {
        if (ticketTraking.ratingAt == null) {
          ...
          bodyRatingMessage +=
            "Digite de 1 al 3 para calificar a Nuestro Personal: \n incremente su atencion al cliente con *CRM-WHASAPO* https://whasapo.com  \n*1* - _Insatisfecho_\n*2* - _Satisfecho_\n*3* - _Muy Satisfecho_\n\n";
...
/*    queueId = null;
    ...
    if (oldQueueId !== queueId && !isNil(oldQueueId) && !isNil(queueId)) {
      ...
      const queueChangedMessage = await wbot.sendMessage(
        ...
        {
          text: "\u200eHas sido transferido a otra Area, pronto iniciaremos tu servicio."
        }
      );
